...
traitrange = pd.read_csv('/media/leon/FREECOM HDD/W-Documents/Sensitivity Analysis/ProsailrangesNuno.csv')
...
x = float(Nth['Fix']), float(CAB['Fix']), float(CAR['Fix']), float(CBP['Fix']), float(CW['Fix']), float(CDM['Fix']), float(Psoil['Fix']), float(LAI['Fix']), float(HOT['Fix']), solar_zenith, solar_azimuth, view_zenith, view_azimuth
x = list(x)
################### RUN MODAL CURVATURE ########################
################################################################

N, chloro, caroten, brown, EWT, LMA, psoil, lai, hot_spot, solar_zenith, solar_azimuth, view_zenith, view_azimuth = x
...
while len(total) < iterat:
        ...
        traits  = pd.DataFrame([np.array([N, chloro, caroten, brown, EWT, LMA, psoil, lai, hot_spot, LIDF[0]])], columns=('N', 'Cab','Car','Cbp','Cw','Cm','Psoil','LAI', 'hot', 'LIDF'))
        total = total.append(pd.concat((traits,spectra),axis=1), ignore_index=True)
        totalnp[row,10:19] = s2outt[1]
        totalnp[row,0:10] = N, chloro, caroten, brown, EWT, LMA, psoil, lai, hot_spot, LIDF[0]
        row = row +1
